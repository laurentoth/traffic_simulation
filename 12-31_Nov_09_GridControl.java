...
public class GridControl{
  ...
  public GridControl(int numIntersectionsInOneDirection){
    ...
    for (int index = 0; index < segment.length; index++) {
      segment[index] = new Segment(segmentCapacity, direction);
      direction++;
      if(direction == 4)
        direction = 0;
    ...
    for (int row = 1; row < numIntersections; row++){
      for (int col = 1; col < numIntersections; col++){
        if (row + 1 < numIntersections) {
          intersection[row][col].setOutbound(intersection[row + 1][col].getInbound(2),
                                         2);
        }
        else {
        ...
        if (col - 1 > 0) {
          intersection[row][col].setOutbound(intersection[row][col - 1].getInbound(3),
            3);
        }
        else {
        ...
        if (row - 1 > 0) {
          intersection[row][col].setOutbound(intersection[row - 1][col].getInbound(0),
            0);
        }
        else {
        ...
        if (col + 1 < numIntersections) {
          intersection[row][col].setOutbound(intersection[row][col + 1].getInbound(1),
            1);
        }
        else {
